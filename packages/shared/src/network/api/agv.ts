...
import { apiGet, apiPost, apiPatch, apiDelete, isMockMode } from "../api-client";
...
export async function addAgv(payload: IAddAgvPayload): Promise<IAgvData> {
  ...
  await new Promise((resolve) => setTimeout(resolve, 500));
  const newAgv: IAgvData = { id: payload.id, x: payload.x, y: payload.y, status: payload.status, timestamp: Date.now() };
...
export async function updateAgv(id: string, payload: Partial<IAgvData & { battery: number; speed: number; task: string }>): Promise<IAgvData> {
  if (!isMockMode()) {
    return apiPatch<IAgvData>(`/agvs/${id}`, payload);
  }
  const agv = mockAgvData.find((a) => a.id === id);
  if (!agv) throw new Error("AGV not found");
  Object.assign(agv, payload, { timestamp: Date.now() });
  return agv;
}

export async function deleteAgv(id: string): Promise<void> {
  if (!isMockMode()) {
    return apiDelete(`/agvs/${id}`);
  }
  const idx = mockAgvData.findIndex((a) => a.id === id);
  if (idx >= 0) mockAgvData.splice(idx, 1);
}
